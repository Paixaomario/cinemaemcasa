...
## 46. Padrão HBO Max confirmado por pesquisa: prévia no hero + rolagem por seção (esta entrega)

Você apontou algo que eu realmente não tinha verificado direito antes
de construir — pesquisei (documentação de apps de TV, incluindo uma
referência que descreve literalmente "banner esmaecido que toca
trailer automaticamente após o foco, só em apps de tela grande") e
confirmei os dois comportamentos que você descreveu. Implementados:

### 1) Prévia centralizada no hero (não mais vídeo em cada capa)
Antes, cada capa focada tocava seu PRÓPRIO vídeo/trailer — um player a
mais rodando por capa, multiplicando chamadas ao TMDB e competindo por
CPU. Agora:
- `components/HeroPreviewContext.tsx` — contexto compartilhado: quando
  qualquer capa (Home, Filmes, Séries, "Títulos semelhantes") recebe
  foco, ela avisa esse contexto em vez de tocar vídeo nela mesma.
- `components/TitleCard.tsx` — não toca mais vídeo/iframe próprio;
  só mostra a capa (com o mesmo reforço de imagem quebrada de antes) e
  avisa o contexto ao focar/desfocar.
- `components/HeroBanner.tsx` — agora tem dois modos: **rotativo**
  (padrão, ninguém focado em nada — continua passando pelos títulos em
  sequência) e **prévia** (assim que uma capa é focada, mostra aquele
  título no hero — capa quase imediata, trailer depois de ~400ms, bem
  mais rápido que o ciclo rotativo de 10s, porque é reação direta ao
  foco). A rotação automática pausa por completo enquanto há uma
  prévia ativa.

Resultado: sempre **um vídeo tocando por vez**, nunca um por capa —
exatamente o padrão HBO Max, e bem mais leve pro Supabase/TMDB.

### 2) Rolagem por seção, uma de cada vez (telas médias/grandes)
`.secao-tela-cheia` (nova classe em `app/globals.css`, com
`scroll-snap-type: y proximity` no `html`) — cada seção/categoria vira
um "andar" de altura de tela cheia; ao rolar, a próxima sobe e ocupa a
tela, a anterior sai de vista, igual Apple tvOS/Android TV. Só ativo em
telas médias/grandes (768px+) — no celular, onde a rolagem é por toque
e o espaço é menor, isso atrapalharia mais do que ajudaria, então lá
continua a rolagem normal, contínua.

### Aviso técnico sobre esta entrega
Durante esta rodada, meu ambiente de trabalho reiniciou e perdi a
cópia local do projeto — você reenviou o zip mais atual e eu continuei
exatamente daquele ponto, sem perder nenhuma correção anterior. Se
notar qualquer coisa de uma entrega anterior que pareça ter "voltado
atrás", me avise imediatamente que reaplico.

## 47. Notas do Agente QA Final
