...
export function RotatingBanner({ items }: RotatingBannerProps) {
  ...
  return (
    <Link
      ...
      className="w-full relative block overflow-hidden bg-transparent ml-0 mr-0 pl-0 pr-0 focus:outline-none" style={{ aspectRatio: '16/9', minHeight: '200px' }}
    >
      {poster ? (
        <div
      ...
      <div className="banner-content-wrapper">
        <div className="space-y-1 sm:space-y-2 md:space-y-3 max-w-full sm:max-w-xl md:max-w-2xl lg:max-w-3xl">
          ...
          <p className="text-xs sm:text-sm md:text-base lg:text-lg xl:text-xl text-slate-300 line-clamp-2 sm:line-clamp-3 hidden md:block">{getDescription(currentItem || {})}</p>
        </div>
      </div>
    </Link>
  )
