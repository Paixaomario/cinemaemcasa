...
export function selectUniqueItems<T extends Record<string, any>>(items: T[], limit: number, usedIds = new Set<string>()) {
  ...
  // Reintroduz a aleatoriedade para garantir conteúdo novo a cada recarga.
  // Embaralha os itens disponíveis antes de selecionar.
...
/**
 * Obtém a lista de "Continuar Assistindo" para o usuário logado.
 * @returns {Promise<any[]>}
 */
export async function getContinueWatchingList() {
  ...
  try {
    const { data, error } = await supabase.rpc('get_continue_watching_list', {
      p_user_id: user.id,
    })
    ...
    console.error('Erro ao buscar lista de "Continuar Assistindo":', error)
    return []
  }
}
...
/**
 * Obtém as preferências visuais para o usuário logado.
 * Retorna as preferências padrão se não houver nenhuma salva.
 */
export const getVisualPreferences = cache(async () => {
  'use server'
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return null
