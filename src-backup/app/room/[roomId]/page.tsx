  ...
  useEffect(() => {
    async function loadRoom(retryCount = 0) {
      ...
      if (room.content_type === 'movie') {
        ...
        const { data: series } = await sb.from('series').select('id_n,titulo,poster,capa,banner,ano,classificacao,genero,rating,trailer,created_at,tmdb_id').eq('id_n', room.content_id).maybeSingle()
