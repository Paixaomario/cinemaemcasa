...
export default function SearchPage() {
  ...
  const performFallbackSearch = async (query: string, genre: string, year: string, type: string) => {
    ...
    // Busca em cinema
    let cinemaQuery = sb.from('cinema').select('id,titulo,poster,backdrop,banner,year,category,rating,duration,duration_seconds,created_at,tmdb_id').limit(50);
    ...
    // Busca em series
    let seriesQuery = sb.from('series').select('id_n,titulo,poster,capa,banner,ano,classificacao,genero,rating,trailer,created_at,tmdb_id').limit(50);
    if (query.trim().length >= 1) {
